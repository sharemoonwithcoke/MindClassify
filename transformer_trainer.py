...
from data_preprocessing import (
    load_dataset, split_dataset, LABEL_NAMES, ID2LABEL, LABEL2ID,
)
...
def fine_tune(
    ...
    """
    Fine-tunes a BERT-family model on the 7-class mental health dataset.

    Returns:
        (best_model, tokenizer, history, test_results)
        test_results: dict with test_df, test_preds, test_labels, test_acc, test_f1
    """
    ...
    print(f"Weighted CE loss : {use_weighted_loss}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"  train  loss={train_loss:.4f}  acc={train_acc:.4f}  "
              f"macro_f1={train_f1:.4f}")
        print(f"  val    loss={val_loss:.4f}  acc={val_acc:.4f}  "
              f"macro_f1={val_f1:.4f}  ← primary metric")
        ...
        if val_f1 > best_val_f1:
            ...
            print(f"  ** Saved best checkpoint (val Macro F1={val_f1:.4f}) → {ckpt_path}")
        else:
            patience_counter += 1
    ...
    # Suicidal Recall — highlight as key safety metric
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fine-tune BERT / MentalBERT for mental health classification"
    )
    parser.add_argument(
        "--model_name", default="bert-base-uncased",
        help="HuggingFace model id. "
             "Step 2: bert-base-uncased  "
             "Step 3: mental/mental-bert-base-uncased",
    )
    ...
    parser.add_argument("--no_weighted_loss", action="store_true",
                        help="Disable weighted cross-entropy (use standard CE)")
    parser.add_argument("--max_train_samples", type=int, default=None,
                        help="Cap training set size for quick tests")
    parser.add_argument("--max_val_samples", type=int, default=None,
                        help="Cap validation set size for quick tests")
    parser.add_argument(
        "--fast", action="store_true",
        help="Quick-test mode: 2 epochs, seq_len=64, batch=32, patience=1, "
             "2 000 train / 500 val samples. Target runtime <15 min on CPU.",
    )
    args = parser.parse_args()

    if args.fast:
        if args.epochs        == 3:   args.epochs           = 2
        if args.max_length    == 512: args.max_length        = 64
        ...
        print(f"  epochs={args.epochs}  max_length={args.max_length}  "
    ...
    model_label = (
        "MentalBERT" if "mental" in args.model_name.lower() else "BERT-base-uncased"
    )

    plot_confusion_matrix(
        test_results["test_labels"], test_results["test_preds"], model_label
    )
    plot_per_class_f1(
        test_results["test_labels"], test_results["test_preds"], model_label
    )
    plot_training_history(
        os.path.join("saved_models", f"{safe}_history.json")
    )
